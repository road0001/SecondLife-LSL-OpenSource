...
integer applyLanguage(){
    setLanguage("_CUR_LANGUAGE", curLanName);
...
default{
    state_entry(){
        hasLanguage=TRUE;
        if(defaultLanguage!=""){
            llMessageLinked(LINK_SET, LAN_MSG_NUM, "LANGUAGE.CHANGE|"+defaultLanguage, llGetOwner());
        }
    }
    ...
    dataserver(key query_id, string data){
        if (query_id == readLanQuery) { // 通过readLanguageNotecards触发读取记事卡事件，按行读取指定语言文本（readLanName）并设置语言。
            while(TRUE){
                string temp=llGetNotecardLineSync(readLanName, readLanLine);
                if(temp!=NAK){
                    data=temp;
                }
                if(data != EOF){
